...
Produce **`js/datastore.js`** — and nothing else. **Output location (loop rule §8):** write it to **`orchestration/outbox/01/js/datastore.js`** and your report to **`orchestration/outbox/01/REPORT.md`**. Do **not** write into the live `js/` tree or touch `data/`, `index.html`, `codex-specs/`, `MASTER-PLAN.md`, `AGENTS.md`, the inbox, or git. Do not create the `.done` sentinel — the runner does that.
